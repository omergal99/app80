...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all app80.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*app80\.sh$/)) {
        return true;
      }

      // Allow all app80agent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*app80agent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@app80.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@app80.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
